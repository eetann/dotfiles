  ...
  config = function()
    ...
    local function create_items(mode)
      ...
      if mode ~= "n" then
        table.insert(
          conversion_dict,
          { prefix = "Convert to ", type = constants.change_type.VISUAL }
        )
      else
        table.insert(
          conversion_dict,
          { prefix = "Convert to ", type = constants.change_type.CURRENT_WORD }
        )
        table.insert(
          conversion_dict,
          { prefix = "Lsp rename ", type = constants.change_type.LSP_RENAME }
        )
      end
