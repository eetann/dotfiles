...
vim.keymap.set(
  "n",
  "s<C-a>",
  "<Cmd>call search('[0-9]','b', line('.'))<CR><C-a>"
)
vim.keymap.set(
  "n",
  "s<C-x>",
  "<Cmd>call search('[0-9]','b', line('.'))<CR><C-x>"
)

vim.keymap.set(
  "n",
  "Y",
  "mz0vg$y`z:delmarks z<CR>",
  { desc = "現在行を改行を含めずにヤンク" }
)
vim.keymap.set(
  "x",
...
vim.keymap.set("n", "sgFF", function()
  local filename_without_ext = vim.fn.expand("%:p")
  vim.fn.setreg("+", filename_without_ext)
  vim.notify("Clipboard << " .. filename_without_ext)
end, { desc = "現在のファイル名(絶対パス)をyank", silent = true })
vim.keymap.set(
  "n",
  "sgFf",
  function()
...
vim.keymap.set(
  "n",
  "<ESC><ESC>",
  "<Cmd>set nohlsearch! hlsearch?<CR>",
  { silent = true }
)
...
vim.keymap.set(
  ...
  "<C-w>k",
  [[<Cmd>wincmd k<CR>]],
  { desc = "go to upper win" }
)
vim.keymap.set(
  "t",
  "<C-w>j",
  [[<Cmd>wincmd j<CR>]],
  { desc = "go to bottom win" }
)
vim.keymap.set(
  "t",
  "<C-w>h",
