...
- 要件定義は「何を作るか（What）」だけを書く。「どう実現するか（How）」は設計フェーズで扱う
    - コードスニペット、具体的な関数名・クラス名、APIのエンドポイント設計などは要件定義に含めない
    - ただしバグ修正など、問題の特定にコード参照が必要な場合は例外として許容する
